...
Theorem distance_R2_triangle_Le :
  forall p x c:set,
    p :e EuclidPlane ->
    x :e EuclidPlane ->
    c :e EuclidPlane ->
    distance_R2 p c <= add_SNo (distance_R2 p x) (distance_R2 x c).
admit.
Qed.
...
let x c1 c2 r1 r2.
assume HxE Hc1 Hc2 Hr1pos Hr2pos Hxball1 Hxball2.
set d1 := distance_R2 x c1.
set d2 := distance_R2 x c2.
claim Hd1R : d1 :e R.
{ exact (distance_R2_in_R x c1 HxE Hc1). }
claim Hd2R : d2 :e R.
{ exact (distance_R2_in_R x c2 HxE Hc2). }
claim Hr1R : r1 :e R.
{ exact (RltE_right 0 r1 Hr1pos). }
claim Hr2R : r2 :e R.
{ exact (RltE_right 0 r2 Hr2pos). }
claim Hd1S : SNo d1.
...
claim Hd1lt : d1 < r1.
{ exact (RltE_lt d1 r1 Hxball1). }
claim Hd2lt : d2 < r2.
{ exact (RltE_lt d2 r2 Hxball2). }
set delta1 := add_SNo r1 (minus_SNo d1).
set delta2 := add_SNo r2 (minus_SNo d2).
...
claim Hdelta1pos : 0 < delta1.
{ exact (SNoLt_minus_pos d1 r1 Hd1S Hr1S Hd1lt). }
claim Hdelta2pos : 0 < delta2.
{ exact (SNoLt_minus_pos d2 r2 Hd2S Hr2S Hd2lt). }
claim Hr1delta1 : add_SNo delta1 d1 = r1.
{ exact (add_SNo_minus_R2' r1 d1 Hr1S Hd1S). }
claim Hr2delta2 : add_SNo delta2 d2 = r2.
{ exact (add_SNo_minus_R2' r2 d2 Hr2S Hd2S). }

apply (SNoLt_trichotomy_or_impred delta1 delta2 Hdelta1S Hdelta2S
  (exists r3:set,
    Rlt 0 r3 /\
    forall p:set, p :e EuclidPlane -> Rlt (distance_R2 p x) r3 ->
      Rlt (distance_R2 p c1) r1 /\ Rlt (distance_R2 p c2) r2)).
- assume Hlt12 : delta1 < delta2.
  set r3 := delta1.
  witness r3.
  apply andI.
  + prove Rlt 0 r3.
    apply (RltI 0 r3 real_0 Hdelta1R).
    exact Hdelta1pos.
  + prove forall p:set, p :e EuclidPlane -> Rlt (distance_R2 p x) r3 ->
      Rlt (distance_R2 p c1) r1 /\ Rlt (distance_R2 p c2) r2.
    let p. assume HpE. assume Hpball.
    claim HdpxR : distance_R2 p x :e R.
    { exact (distance_R2_in_R p x HpE HxE). }
    claim Hdpc1R : distance_R2 p c1 :e R.
    { exact (distance_R2_in_R p c1 HpE Hc1). }
    claim Hdpc2R : distance_R2 p c2 :e R.
    { exact (distance_R2_in_R p c2 HpE Hc2). }
    claim HdpxS : SNo (distance_R2 p x).
    { exact (real_SNo (distance_R2 p x) HdpxR). }
    claim Hdpc1S : SNo (distance_R2 p c1).
    { exact (real_SNo (distance_R2 p c1) Hdpc1R). }
    claim Hdpc2S : SNo (distance_R2 p c2).
    { exact (real_SNo (distance_R2 p c2) Hdpc2R). }
    claim Hr3R : r3 :e R.
    { exact Hdelta1R. }
    claim Hr3S : SNo r3.
    { exact (real_SNo r3 Hr3R). }
    claim Hdpxlt : distance_R2 p x < r3.
    { exact (RltE_lt (distance_R2 p x) r3 Hpball). }
    claim Htri1 : distance_R2 p c1 <= add_SNo (distance_R2 p x) d1.
    { exact (distance_R2_triangle_Le p x c1 HpE HxE Hc1). }
    claim Htri2 : distance_R2 p c2 <= add_SNo (distance_R2 p x) d2.
    { exact (distance_R2_triangle_Le p x c2 HpE HxE Hc2). }
    claim Hd1r3le : r3 <= delta1.
    { exact (SNoLe_ref delta1). }
    claim Hd2r3le : r3 <= delta2.
    { exact (SNoLtLe r3 delta2 Hlt12). }
    (** bound for c1 **)
    claim Hsum1lt : add_SNo (distance_R2 p x) d1 < r1.
    { claim Hcom1 : add_SNo (distance_R2 p x) d1 = add_SNo d1 (distance_R2 p x).
      { exact (add_SNo_com (distance_R2 p x) d1 HdpxS Hd1S). }
      claim Hcom2 : add_SNo d1 r3 = add_SNo r3 d1.
      { exact (add_SNo_com d1 r3 Hd1S Hr3S). }
      claim Hltadd : add_SNo d1 (distance_R2 p x) < add_SNo d1 r3.
      { exact (add_SNo_Lt2 d1 (distance_R2 p x) r3 Hd1S HdpxS Hr3S Hdpxlt). }
      claim Hleadd : add_SNo d1 r3 <= r1.
      { claim Hle1 : add_SNo r3 d1 <= add_SNo delta1 d1.
        { exact (add_SNo_Le1 r3 d1 delta1 Hr3S Hd1S Hdelta1S Hd1r3le). }
        claim Hle2 : add_SNo r3 d1 <= r1.
        { claim Hle2' : add_SNo r3 d1 <= add_SNo delta1 d1.
          { exact Hle1. }
          rewrite Hr1delta1.
          exact Hle2'. }
        rewrite (add_SNo_com d1 r3 Hd1S Hr3S).
        exact Hle2. }
      claim Hlt1 : add_SNo d1 (distance_R2 p x) < r1.
      { exact (SNoLtLe_tra (add_SNo d1 (distance_R2 p x)) (add_SNo d1 r3) r1
                (real_SNo (add_SNo d1 (distance_R2 p x)) (real_add_SNo d1 Hd1R (distance_R2 p x) HdpxR))
                (real_SNo (add_SNo d1 r3) (real_add_SNo d1 Hd1R r3 Hr3R))
                Hr1S
                Hltadd
                Hleadd). }
      rewrite Hcom1.
      exact Hlt1. }
    claim Hdpc1lt : distance_R2 p c1 < r1.
    { exact (SNoLeLt_tra (distance_R2 p c1) (add_SNo (distance_R2 p x) d1) r1 Hdpc1S
              (real_SNo (add_SNo (distance_R2 p x) d1) (real_add_SNo (distance_R2 p x) HdpxR d1 Hd1R))
              Hr1S
              Htri1
              Hsum1lt). }
    claim HRc1 : Rlt (distance_R2 p c1) r1.
    { exact (RltI (distance_R2 p c1) r1 Hdpc1R Hr1R Hdpc1lt). }
    (** bound for c2 **)
    claim Hsum2lt : add_SNo (distance_R2 p x) d2 < r2.
    { claim Hcom1 : add_SNo (distance_R2 p x) d2 = add_SNo d2 (distance_R2 p x).
      { exact (add_SNo_com (distance_R2 p x) d2 HdpxS Hd2S). }
      claim Hcom2 : add_SNo d2 r3 = add_SNo r3 d2.
      { exact (add_SNo_com d2 r3 Hd2S Hr3S). }
      claim Hltadd : add_SNo d2 (distance_R2 p x) < add_SNo d2 r3.
      { exact (add_SNo_Lt2 d2 (distance_R2 p x) r3 Hd2S HdpxS Hr3S Hdpxlt). }
      claim Hleadd : add_SNo d2 r3 <= r2.
      { claim Hle1 : add_SNo r3 d2 <= add_SNo delta2 d2.
        { exact (add_SNo_Le1 r3 d2 delta2 Hr3S Hd2S Hdelta2S Hd2r3le). }
        claim Hle2 : add_SNo r3 d2 <= r2.
        { claim Hle2' : add_SNo r3 d2 <= add_SNo delta2 d2.
          { exact Hle1. }
          rewrite Hr2delta2.
          exact Hle2'. }
        rewrite (add_SNo_com d2 r3 Hd2S Hr3S).
        exact Hle2. }
      claim Hlt1 : add_SNo d2 (distance_R2 p x) < r2.
      { exact (SNoLtLe_tra (add_SNo d2 (distance_R2 p x)) (add_SNo d2 r3) r2
                (real_SNo (add_SNo d2 (distance_R2 p x)) (real_add_SNo d2 Hd2R (distance_R2 p x) HdpxR))
                (real_SNo (add_SNo d2 r3) (real_add_SNo d2 Hd2R r3 Hr3R))
                Hr2S
                Hltadd
                Hleadd). }
      rewrite Hcom1.
      exact Hlt1. }
    claim Hdpc2lt : distance_R2 p c2 < r2.
    { exact (SNoLeLt_tra (distance_R2 p c2) ((distance_R2 p x) + d2) r2 Hdpc2S
              (real_SNo ((distance_R2 p x) + d2) (real_add_SNo (distance_R2 p x) HdpxR d2 Hd2R))
              Hr2S
              Htri2
              Hsum2lt). }
    claim HRc2 : Rlt (distance_R2 p c2) r2.
    { exact (RltI (distance_R2 p c2) r2 Hdpc2R Hr2R Hdpc2lt). }
    apply andI.
    - exact HRc1.
    - exact HRc2.
- assume Heq : delta1 = delta2.
  set r3 := delta1.
  witness r3.
  apply andI.
  + prove Rlt 0 r3.
  ...
  + prove forall p:set, p :e EuclidPlane -> Rlt (distance_R2 p x) r3 ->
      Rlt (distance_R2 p c1) r1 /\ Rlt (distance_R2 p c2) r2.
    let p. assume HpE. assume Hpball.
    claim Hlt12 : delta1 < delta2 \/ delta2 < delta1 \/ delta1 = delta2.
    { apply orIR. apply orIR. exact Heq. }
    apply (Hlt12 (Rlt (distance_R2 p c1) r1 /\ Rlt (distance_R2 p c2) r2)).
    - assume Hcontra. apply FalseE. exact ((SNoLt_irref delta1) Hcontra).
    - assume Hcontra. apply FalseE. exact ((SNoLt_irref delta2) Hcontra).
    - assume _ : delta1 = delta2.
      (** reuse first branch reasoning with r3=delta1 and r3<=delta2 by equality **)
      claim Hr3R : r3 :e R.
      { exact Hdelta1R. }
      claim Hr3S : SNo r3.
      { exact (real_SNo r3 Hr3R). }
      ...
      claim Hdpc2S : SNo (distance_R2 p c2).
      { exact (real_SNo (distance_R2 p c2) Hdpc2R). }
      claim Hsum1lt : (distance_R2 p x) + d1 < r1.
      { claim Hcom1 : (distance_R2 p x) + d1 = d1 + (distance_R2 p x).
        { exact (add_SNo_com (distance_R2 p x) d1 HdpxS Hd1S). }
        claim Hcom2 : d1 + r3 = r3 + d1.
        { exact (add_SNo_com d1 r3 Hd1S Hr3S). }
        claim Hltadd : d1 + (distance_R2 p x) < d1 + r3.
        { exact (add_SNo_Lt2 d1 (distance_R2 p x) r3 Hd1S HdpxS Hr3S Hdpxlt). }
        claim Hleadd : d1 + r3 <= r1.
        { claim Hle1 : r3 + d1 <= delta1 + d1.
          { exact (add_SNo_Le1 delta1 d1 delta1 Hdelta1S Hd1S Hdelta1S (SNoLe_ref delta1)). }
          claim Hle2 : r3 + d1 <= r1.
          { rewrite <- Hr1delta1. exact Hle1. }
          rewrite <- Hcom2. exact Hle2. }
        claim Hlt1 : d1 + (distance_R2 p x) < r1.
        { exact (SNoLtLe_tra (d1 + (distance_R2 p x)) (d1 + r3) r1
                  (real_SNo (d1 + (distance_R2 p x)) (real_add_SNo d1 Hd1R (distance_R2 p x) HdpxR))
                  (real_SNo (d1 + r3) (real_add_SNo d1 Hd1R r3 Hr3R))
                  Hr1S
                  Hltadd
                  Hleadd). }
        rewrite Hcom1. exact Hlt1. }
      claim Hdpc1lt : distance_R2 p c1 < r1.
      { exact (SNoLeLt_tra (distance_R2 p c1) ((distance_R2 p x) + d1) r1 Hdpc1S
                (real_SNo ((distance_R2 p x) + d1) (real_add_SNo (distance_R2 p x) HdpxR d1 Hd1R))
                Hr1S Htri1 Hsum1lt). }
      claim HRc1 : Rlt (distance_R2 p c1) r1.
      { exact (RltI (distance_R2 p c1) r1 Hdpc1R Hr1R Hdpc1lt). }
      claim Hsum2lt : (distance_R2 p x) + d2 < r2.
      { claim Hcom1 : (distance_R2 p x) + d2 = d2 + (distance_R2 p x).
        { exact (add_SNo_com (distance_R2 p x) d2 HdpxS Hd2S). }
        claim Hcom2 : d2 + r3 = r3 + d2.
        { exact (add_SNo_com d2 r3 Hd2S Hr3S). }
        claim Hltadd : d2 + (distance_R2 p x) < d2 + r3.
        { exact (add_SNo_Lt2 d2 (distance_R2 p x) r3 Hd2S HdpxS Hr3S Hdpxlt). }
        claim Hr3le2 : r3 <= delta2.
        { rewrite <- Heq. exact (SNoLe_ref delta1). }
        claim Hleadd : d2 + r3 <= r2.
        { claim Hle1 : r3 + d2 <= delta2 + d2.
          { exact (add_SNo_Le1 r3 d2 delta2 Hr3S Hd2S Hdelta2S Hr3le2). }
          claim Hle2 : r3 + d2 <= r2.
          { rewrite <- Hr2delta2. exact Hle1. }
          rewrite <- Hcom2. exact Hle2. }
        claim Hlt1 : d2 + (distance_R2 p x) < r2.
        { exact (SNoLtLe_tra (d2 + (distance_R2 p x)) (d2 + r3) r2
                  (real_SNo (d2 + (distance_R2 p x)) (real_add_SNo d2 Hd2R (distance_R2 p x) HdpxR))
                  (real_SNo (d2 + r3) (real_add_SNo d2 Hd2R r3 Hr3R))
                  Hr2S Hltadd Hleadd). }
        rewrite Hcom1. exact Hlt1. }
      claim Hdpc2lt : distance_R2 p c2 < r2.
      { exact (SNoLeLt_tra (distance_R2 p c2) ((distance_R2 p x) + d2) r2 Hdpc2S
      ...
      - exact HRc1.
      - exact HRc2.
- assume Hlt21 : delta2 < delta1.
  set r3 := delta2.
  witness r3.
  apply andI.
  + prove Rlt 0 r3.
    apply (RltI 0 r3 real_0 Hdelta2R).
    exact Hdelta2pos.
  + prove forall p:set, p :e EuclidPlane -> Rlt (distance_R2 p x) r3 ->
      Rlt (distance_R2 p c1) r1 /\ Rlt (distance_R2 p c2) r2.
    let p. assume HpE. assume Hpball.
    ...
    claim Htri2 : distance_R2 p c2 <= add_SNo (distance_R2 p x) d2.
    { exact (distance_R2_triangle_Le p x c2 HpE HxE Hc2). }
    claim Hr3le1 : r3 <= delta1.
    { exact (SNoLtLe r3 delta1 Hlt21). }
    claim Hr3le2 : r3 <= delta2.
    { exact (SNoLe_ref delta2). }
    claim Hsum1lt : (distance_R2 p x) + d1 < r1.
    { claim Hcom1 : (distance_R2 p x) + d1 = d1 + (distance_R2 p x).
      { exact (add_SNo_com (distance_R2 p x) d1 HdpxS Hd1S). }
      claim Hcom2 : d1 + r3 = r3 + d1.
      ...
      claim Hleadd : d1 + r3 <= r1.
      { claim Hle1 : r3 + d1 <= delta1 + d1.
        { exact (add_SNo_Le1 r3 d1 delta1 Hr3S Hd1S Hdelta1S Hr3le1). }
        claim Hle2 : r3 + d1 <= r1.
        { rewrite <- Hr1delta1. exact Hle1. }
        rewrite <- Hcom2.
        exact Hle2. }
...
Definition box_basis : set -> set -> set := fun I Xi =>
  {B :e Power (product_space I Xi) |
    exists U:set, function_on U I (Union Xi) /\
      (forall i:set, i :e I -> apply_fun U i :e apply_fun Xi i) /\
